...
const parseType = (source, hideModal, manifestURL) => {
    let { 
...
const Modal = ({ workDetail, hideModal, doc_id, show, manifestURL }) => {
    console.log('props from MODAL', manifestURL)
    ...
    if(manifestURL.length > 0) {
        ...
        console.log('no url?', manifestURL)
        localStorage.setItem("manifestURL", "")
    }
    
    if(Object.keys(workDetail).length > 0) {
        data = parseType(workDetail._source, hideModal, manifestURL)
    } else {
