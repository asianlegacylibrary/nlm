...
class Modal extends Component {
    ...
    parseModalDetails = ({ workDetail, workResources, label, t }) => {
        ...
        if (workResources.length > 0) {
            resources = bdrcResources(workResources, label)
        }
...
const mapStateToProps = state => ({
    ...
    workDetail:
        state.detailsItem.isFetching || state.modal.modalID === 0
            ? {}
            : state.detailsItem.item.hits.hits[0],
    workResources:
        state.detailsRelated.items.length === 0
            ? {}
            : state.detailsRelated.items.hits.hits,
    numberVolumes:
        state.detailsItem.isFetching || state.modal.modalID === 0
            ? null
            : state.detailsItem.item.hits.hits[0]._source.workNumberOfVolumes,
...
export default React.memo(connect(mapStateToProps)(withN))
